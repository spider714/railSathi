...
export function JourneyCard({
  ...
  return (
    <div
      className={cn(
        'glass-panel relative overflow-hidden rounded-3xl p-4 sm:p-6 shadow-glass transition-all duration-300',
        className
      )}
    >
      {/* Header Bar */}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="flex items-center gap-3.5 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-slate-800/60 dark:bg-slate-900/50">
          ...
          <div>
            <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">
              {isHi ? 'वर्तमान / पिछला स्टेशन' : 'Current / Last Station'}
            </span>
            <p className="font-semibold text-slate-900 dark:text-white">
              {journey.currentStation?.name || journey.previousStation?.name || (isHi ? 'मार्ग में' : 'In Transit')}
            </p>
            {journey.currentStation?.platform && (
              <span className="text-xs text-slate-500 dark:text-slate-400">
                {isHi ? `प्लेटफ़ॉर्म ${journey.currentStation.platform}` : `Platform ${journey.currentStation.platform}`}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3.5 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-slate-800/60 dark:bg-slate-900/50">
          ...
          <div>
            <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">
              {isHi ? 'लाइव गति' : 'Live Speed'}
            </span>
            <div className="flex items-baseline gap-1">
              <span className="font-mono text-xl font-bold text-slate-900 dark:text-white">
                {journey.speedKmh}
              </span>
              <span className="text-xs font-semibold text-slate-500">{isHi ? 'किमी/घंटा' : 'km/h'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50/70 p-4 dark:border-slate-800/60 dark:bg-slate-900/50">
          <div>
            <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">
              {isHi ? 'तय की गई दूरी' : 'Distance Covered'}
            </span>
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">
              {formatDistance(journey.remainingDistanceKm)} {isHi ? 'शेष' : 'remaining'}
            </span>
          </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
        <span>{isHi ? 'प्रत्येक 30 सेकंड में स्वतः अपडेट होता है' : 'Auto-refreshes every 30 seconds'}</span>
        <span>{isHi ? 'अद्यतन' : 'Updated'} {formatTimeAgo(journey.lastUpdated)}</span>
      </div>
    </div>
  );
}
