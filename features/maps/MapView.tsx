  ...
  useEffect(() => {
    ...
    if (map.getSource('route')) {
      ...
      map.addLayer({
        ...
        paint: { 'line-color': '#f43f5e', 'line-width': 12, 'line-opacity': 0.2, 'line-blur': 8 },
      });
      ...
      map.addLayer({
        ...
        source: 'route',
        layout: { 'line-join': 'round', 'line-cap': 'round' },
        paint: { 'line-color': '#fb7185', 'line-width': 3.5 },
      });
    }
    ...
    if (!markerRef.current) {
      ...
      el.innerHTML = `
        <div class="relative flex items-center justify-center w-10 h-10">
          ...
          <div class="relative flex h-9 w-9 items-center justify-center rounded-full bg-rose-500 text-white border-2 border-white shadow-lg text-lg">
            🚄
          </div>
        </div>`;
      ...
      const popup = new maplibregl.Popup({ offset: 16, closeButton: false }).setHTML(`
        <div class="p-2 font-sans">
          ...
          <div class="text-[11px] font-semibold text-rose-600 mt-0.5">
    ...
    journey.stations.forEach((st) => {
      ...
      el.innerHTML = `<div class="rounded-full border-2 border-white shadow-sm cursor-pointer transition-transform hover:scale-150 ${
        isCurrent ? 'h-4 w-4 bg-rose-500 ring-4 ring-rose-500/30' : isPassed ? 'h-2.5 w-2.5 bg-emerald-500' : 'h-2.5 w-2.5 bg-slate-400'
      }"></div>`;
    ...
    const pois = getJourneyPois(journey);
    pois.forEach((poi) => {
      ...
      el.innerHTML = `<div class="flex h-6 w-6 items-center justify-center rounded-full bg-slate-900/80 text-xs border border-white/40 shadow-md cursor-pointer transition-transform hover:scale-125">${icon}</div>`;

      const popup = new maplibregl.Popup({ offset: 10, closeButton: false }).setHTML(`
        <div class="p-2 font-sans max-w-[180px]">
          <div class="font-bold text-xs text-rose-600">${poi.name}</div>
          <div class="text-[10px] text-gray-500 mt-0.5 line-clamp-2">${poi.description}</div>
          ${poi.localFoodSpecialty ? `<div class="text-[10px] text-amber-600 font-semibold mt-1">🍱 ${poi.localFoodSpecialty}</div>` : ''}
        </div>`);
