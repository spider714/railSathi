...
export function AnalyticsDashboard({ journey }: AnalyticsDashboardProps) {
    ...
    return (
      <div className="space-y-4">
        <div className="glass-panel rounded-2xl p-5 text-xs text-slate-400 text-center">
          {isHi ? 'OpenTopography से यात्रा विश्लेषण व ऊंचाई प्रोफ़ाइल की गणना हो रही है…' : 'Computing journey analytics & elevation profile from OpenTopography…'}
        </div>
      </div>
    );
  }
  ...
  const statsList = [
    { icon: Route, color: 'text-rail-blue', bg: 'bg-rail-blue/10', label: isHi ? 'कुल दूरी' : 'Total Distance', value: analytics.totalDistanceKm, suffix: isHi ? ' किमी' : ' km' },
    { icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-500/10', label: isHi ? 'उच्चतम बिंदु' : 'Highest Point', value: analytics.highestElevationM, suffix: isHi ? ' मी' : ' m' },
    { icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-500/10', label: isHi ? 'तय दूरी' : 'Covered', value: analytics.distanceCoveredKm, suffix: isHi ? ' किमी' : ' km' },
    { icon: Clock, color: 'text-rose-500', bg: 'bg-rose-500/10', label: isHi ? 'विलंब' : 'Delay', value: journey.delayMinutes, suffix: isHi ? ' मिनट' : ' min' },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {statsList.map(({ icon: Icon, color, bg, label, value, suffix }) => (
          <motion.div
            key={label}
      ...
      <div className="glass-panel rounded-3xl p-6 shadow-glass space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2">
            ...
            {isHi ? 'प्रति-स्टेशन विलंब इतिहास' : 'Per-Station Delay History'}
          </h3>
        ...
        {delayData.length > 0 ? (
              ...
              return (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="w-14 text-[11px] font-mono font-semibold text-right flex-shrink-0 text-slate-700 dark:text-slate-200">
                    {d.delayMinutes > 0 ? `+${d.delayMinutes}m` : (isHi ? 'समय पर' : 'On time')}
                  </span>
                </div>
              );
            })}
          ...
          <p className="text-xs text-slate-400">{isHi ? 'कोई विलंब इतिहास उपलब्ध नहीं है।' : 'No delay history available.'}</p>
        )}
      </div>
    </div>
  );
