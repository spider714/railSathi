  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 font-bold text-lg text-slate-900 dark:text-white">
        ...
        <span>{isHi ? 'भूभाग व मार्ग की विशेषताएं' : 'Terrain & Points of Interest'}</span>
      ...
      {loading && (
        <div className="glass-panel flex items-center gap-3 rounded-2xl p-5 text-sm text-slate-500">
          ...
          <span>{isHi ? 'मार्ग में पुल, नदियां व भूभाग लोड हो रहे हैं…' : 'Fetching bridges, rivers & terrain features along route…'}</span>
        </div>
      )}

      {error && !loading && (
        <div className="glass-panel rounded-2xl p-5 text-sm text-slate-500">
          {isHi ? 'भूभाग डेटा अस्थायी रूप से अनुपलब्ध है।' : 'Terrain data is temporarily unavailable. Try again later.'}
        </div>
      )}
      ...
      {!loading && !error && features.length === 0 && (
        <div className="glass-panel flex items-center gap-3 rounded-2xl p-5 text-sm text-slate-500">
          ...
          <span>{isHi ? 'इस मार्ग पर कोई विशेष भूभाग नहीं मिला।' : 'No terrain features found along this route.'}</span>
        </div>
      )}
